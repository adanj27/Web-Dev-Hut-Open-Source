...
export function EditorTextArea({
  ...
  const formatCode = async (curTab) => {
    let formattedCode
    if (curTab == 0) {
      formattedCode = await prettier.format(html, {
        parser: 'html',
        plugins: ['htmlParser'],
      })
    ...
    if (curTab == 1) {
      formattedCode = await prettier.format(css, {
        parser: 'cssp',
        plugins: [cssp],
      })
  ...
  return (
    <div className="drop-shadow-lg border-solid border-2 border-white rounded-md p-4">
